...
This script trains a matrix factorization model using SVD from scikit-surprise.
...
from config import COLLABORATIVE_PARAMS, METRICS_DIR, MODELS_DIR, PROCESSED_DIR, TEST_MODE, print_config
...
def main():
    ...
    params = {**COLLABORATIVE_PARAMS, "random_state": 42}
    ...
    with mlflow.start_run():
        ...
        print("\nInitializing model...")
        model = CollaborativeFilteringModel(**params)
        ...
        with open(METRICS_DIR / "collaborative_results.json", "w") as f:
            json.dump(results, f, indent=2)

        print(f"\n✓ Results saved to {METRICS_DIR / 'collaborative_results.json'}")

        model_path = SAVED_MODELS_DIR / "collaborative_model"
